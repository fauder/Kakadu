...
void main()
{
    ...
    vs_out.varying_tangent_world_space   = vec4( normalize( tangent.xyz * mat3x3( world_transform ) ), 0.0 );
    vs_out.varying_normal_world_space    = vec4( normalize( normal * world_transform_for_normals ), 0.0 );
    vs_out.varying_bitangent_world_space = vec4( tangent.w * normalize( cross( vs_out.varying_normal_world_space.xyz, vs_out.varying_tangent_world_space.xyz ) ), 0.0 );
